    ...
    <el-table :data="allLabs" stripe style="width: 100%" max-height="600">
      ...
      <el-table-column fixed="right" label="操作" min-width="120">
        <template #default="scope">
          <el-button link type="primary" size="small" @click="handleRepair(scope.row.labId, scope.row.labAdminId)">报修</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
const handleRepair = async (labId: number, labAdminId: number) => {
  try {
    if (!userInfo.value) {
